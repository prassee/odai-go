...
func OnTumblingWindow(wg *sync.WaitGroup, intStrm chan TickData , window TumblingWindow, fn func(batch []TickData)) {
		defer wg.Done()
		data := []TickData{}
		for i := range intStrm {
...
func OnSlidingWindow(wg *sync.WaitGroup,intStrm chan TickData, window SlidingWindow, fn func([]TickData)) {
		defer wg.Done()
